...
.option('dbtable', 'test_table_raw').load())

df=df.withColumn('start_date',current_time).withColumn('update_date',current_time).filter(col('batch_id')==2738)
...
.option('user', 'postgres')
.option('password', 'postgres')
.option('dbtable', 'test_table_bronze').mode('overwrite').save())
...
.option('db', 'MAHENDER')
.option('schema', 'TEST')
.option('dbtable', 'test_table_bronze')
.mode('overwrite').save())